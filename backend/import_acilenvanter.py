...
# ---
# Bu script, acilenvanter.csv dosyasındaki cihazları toplu olarak veritabanına ekler.
# Eksik alanları '-' ile doldurur, department yoksa otomatik oluşturur.
# Her cihaz için CalibrationLog da ekler.
# Kullanım: python import_acilenvanter.py
# ---

CSV_PATH = os.path.join(os.path.dirname(__file__), 'acilenvanter.csv')
...
def main():
    with open(CSV_PATH, encoding='utf-8') as f:
        ...
        for idx, row in enumerate(reader):
            ...
            print(f"{device.name} ({device.serial}) eklendi. [Yeni: {created}] | Inventory No: {inventory_no}")
